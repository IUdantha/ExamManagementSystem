...
function Function3() {
  ...
  <body onload="startTime(); visibility();">
   ...
   <div class="login-box">
        <div class="heading">View Examiner List</div>
        <form name="formal" method="post" action="ViewEX" autocomplete="off">
	...
	<c:forEach var="exam" items="${examinerDetails}"> 
	
	
	
		 	<!-- Assign data to variables -->
			<c:set var="id" value="${exam.exID}"/>
			<c:set var="faculty" value="${exam.faculty}"/>
			<c:set var="year" value="${exam.year}"/>
			<c:set var="filename" value="${exam.filename}"/>
			
			
			<!-- Give the page name we need to redirect the data into -->
			<c:url value="updateExaminer.jsp" var = "examinerUpdate">
	...
	<!-- The name given in set attribute in servlet file -->
			<center><a href="${examinerUpdate}">
				<button  id="button1"  class="button button1" >EDIT</button>
			</a>
			<a href="deleteExaminer.jsp">
				<button  id="button2" class="button button2">DELETE</button>
			</a></center>
			
		<h3>Faculty   :  ${exam.faculty} <br/><!-- It calls the getters -->
		Year      :  ${exam.year} <br/></h3>
		<br><br>
		<h3>Examiner List :</h3>
		<iframe src="Examiner/${exam.filename}" width="100%" height="600px">
   		</iframe>

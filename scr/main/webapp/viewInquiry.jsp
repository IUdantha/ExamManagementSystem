...
function Function3() {
  ...
  <body onload="startTime(); visibility();">
   ...
   <div class="login-box">
        <div class="heading">View Inquiry</div>
	...
	<c:forEach var="in" items="${inquiryDetails}"> 
	
		 	<!-- Assign data to variables -->
			<c:set var="id" value="${in.ttID}"/>
			<c:set var="faculty" value="${in.faculty}"/>
			<c:set var="year" value="${in.year}"/>
			<c:set var="filename" value="${in.filename}"/>
			
			
			<!-- Give the page name we need to redirect the data into -->
			<c:url value="updateInquiry.jsp" var = "inquiryUpdate">
	...
	<!-- The name given in set attribute in servlet file -->
			<center><a href="${inquiryUpdate}">
				<button  id="button1"  class="button button1" >EDIT</button>
			</a>
			<a href="deleteInquiry.jsp">
		...
		<c:forEach var="in" items="${inquiryDetails}"> 	
			...
			Email   :  ${in.email} <br/>
			RegNo   :  ${in.number} <br/>
			Faculty   :  ${in.faculty} <br/><!-- It calls the getters -->
			Year      :  ${in.year} <br/>
			Message   :  ${in.message} <br/>
			Inquiry Type   :  ${in.inquirytype} <br/>
			</h3>
			<br><br>
			
			<img src="inquiry/${in.filename}" width="100%" height="600px"/>

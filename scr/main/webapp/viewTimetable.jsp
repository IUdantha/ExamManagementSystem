...
function Function3() {
  ...
  <body onload="startTime(); visibility();">
   ...
   <div class="login-box">
        <div class="heading">View Timetable</div>
        <form name="formal" method="post" action="Viewtt" autocomplete="off">
	...
	<c:forEach var="timet" items="${timeTDetails}"> 
	
		 	<!-- Assign data to variables -->
			<c:set var="id" value="${timet.ttID}"/>
			<c:set var="faculty" value="${timet.faculty}"/>
			<c:set var="year" value="${timet.year}"/>
			<c:set var="filename" value="${timet.filename}"/>
			
			
			<!-- Give the page name we need to redirect the data into -->
			<c:url value="updateTimetable.jsp" var = "timetableUpdate">
	...
	<!-- The name given in set attribute in servlet file -->
			<center><a href="${timetableUpdate}">
				<button  id="button1"  class="button button1" >EDIT</button>
			</a>
			<a href="deleteTimetable.jsp">
				<button  id="button2" class="button button2">DELETE</button>
			</a></center>
	</c:forEach>
	
		<c:forEach var="timet" items="${timeTDetails}"> 	
			<h3>Faculty   :  ${timet.faculty} <br/><!-- It calls the getters -->
			Year      :  ${timet.year} <br/></h3>
			<br><br>
			<h3>Timetable :</h3>
			<iframe src="timetable/${timet.filename}" width="100%" height="600px">
	   		</iframe>

...
function Function3() {
  ...
  <body onload="startTime()">
	...
	<div class="login-box">
        <form name="formal" method="post" action="submitAnswer" autocomplete="off">
        
			<div class="get-std">
	        		<label for="Name">Quiz :</label>
	        			<input class="get-std" type="text" id="" name="table" value="${table}" readonly><br><br>
	            	<label for="Name">Student Number      </label>
	                	<input class="get-std" type="text" id="sid" name="sid" placeholder="Enter Student Number" required/>
	        </div><br><br><br>
		 	<c:forEach var="q" items="${loadQuestion}">
		 		
		        <div class="heading">${q.qid}) ${q.question} </div>
		        
		        
				<div class="form-input">
		                	<input type="radio" id="" name="${q.qid}" value="${q.opt1}" required>   ${q.opt1}<br>
		                	<input type="radio" id="" name="${q.qid}" value="${q.opt2}" required>   ${q.opt2}<br>
		                	<input type="radio" id="" name="${q.qid}" value="${q.opt3}" required>   ${q.opt3}<br>
		                	<input type="radio" id="" name="${q.qid}" value="${q.opt4}" required>   ${q.opt4}<br><br><br>
		                	
		            	
		        </div>
		                
			</c:forEach>
        <button name="submit" id="login-btn">SUBMIT</button>
   		</form>
